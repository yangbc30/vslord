...
class GameState(MyWidget):
    ...
    def __init__(self, room_owner, player_info, current_player, node, *args, **kwargs):
        """保存gamestate基础信息, 初始化一些组件
        """
        super(GameState, self).__init__(*args, **kwargs)

        Me(name=room_owner, info=player_info[room_owner], owner=self, client=node)
        for name in player_info:
            if name != room_owner:
                Me(name=name, info=player_info[name], owner=self)
        Desk(owner=self)
        self.me = room_owner
...
if __name__ == "__main__":
    ...
    root = Tk()
    gamestate = GameState("yangbc", CLIENT.player_info, current_player="yangbc", node=CLIENT)
